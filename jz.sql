...
CREATE TABLE services (
    ...
    unit ENUM('hour', 'square_meter', 'item', 'fixed') NOT NULL COMMENT '计价单位',
...
CREATE TABLE orders (
    ...
    time_slot ENUM('morning', 'afternoon', 'evening', 'full_day') NOT NULL COMMENT '服务时段',
    ...
    order_status ENUM('pending', 'confirmed', 'assigned', 'ongoing', 'completed', 'cancelled') DEFAULT 'pending' COMMENT '订单状态',
...
CREATE TABLE complaints (
    ...
    complaint_type ENUM('service', 'punctuality', 'attitude', 'other') NOT NULL COMMENT '投诉类型',
    complaint_text TEXT NOT NULL COMMENT '投诉内容',
    status ENUM('pending', 'investigating', 'resolved', 'rejected') DEFAULT 'pending' COMMENT '处理状态',
...
CREATE TABLE payments (
    ...
    order_id VARCHAR(20) NOT NULL COMMENT '订单ID',
    user_id INT NOT NULL COMMENT '用户ID',
    payment_method ENUM('wechat', 'alipay', 'balance', 'card') NOT NULL COMMENT '支付方式',
    payment_amount DECIMAL(10,2) NOT NULL COMMENT '支付金额',
    payment_status ENUM('pending', 'processing', 'success', 'failed') DEFAULT 'pending' COMMENT '支付状态',
...
CREATE TABLE notifications (
    ...
    user_id INT NOT NULL COMMENT '用户ID',
    notification_type ENUM('order', 'system', 'promotion') NOT NULL COMMENT '消息类型',
